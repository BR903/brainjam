#!/usr/bin/env python
#
# Usage: mkgames.py GAMEDATA.TXT GAMEDATA.BIN
#
...
def makeconfig(line, outfile):
    id, deck, best = line.split()
    seq = deckseq(deck)
    seq.insert(0, (int(best) - 52, 7))
    outfile.write(outputbits(seq))
...
def translate(infilename, outfilename):
    ...
    for line in infile.readlines():
        ...
        if not line or line.startswith('#'):
            continue
        makeconfig(line, outfile)
    infile.close()
    outfile.close()


if len(sys.argv) != 3:
    sys.exit('Usage: mkgames.py GAMEDATA.TXT GAMEDATA.BIN')
