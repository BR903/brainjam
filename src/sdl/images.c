...
/* The next two images are collections of smaller images, stored in
 * the fashion of a sprite sheet. The first file contains indicator
 * graphics, and the second file contains button graphics. Individual
 * images are identified using values defined in sdl/alertids.h and
 * sdl/biconids.h, and are located via the SDL_Rect arrays defined
 * below. The button graphics image is freed after all of the buttons
 * have been created.
 */
INCBIN("sdl/alerts.png", pngalerts, pngalerts_end);
INCBIN("sdl/bicons.png", pngbicons, pngbicons_end);
...
#include "sdl/alertpos.h"
};

static SDL_Rect const biconrects[] = {
#include "sdl/biconpos.h"
};
 ...
 */
static SDL_Surface *biconsurface = NULL;
...
void initializeimages(void)
{
    ...
    initdeck();
    biconsurface = pngtosurface(pngbicons);
    image = pngtosurface(pngalerts);
    alerttexture = SDL_CreateTextureFromSurface(_graph.renderer, image);
    SDL_FreeSurface(image);
}

/* Make a copy of a button icon.
 */
SDL_Surface *getbuttonicon(int id)
{
    SDL_Surface *image;

    image = SDL_CreateRGBSurface(0, biconrects[id].w, biconrects[id].h,
                                 biconsurface->format->BitsPerPixel,
                                 biconsurface->format->Rmask,
                                 biconsurface->format->Gmask,
                                 biconsurface->format->Bmask,
                                 biconsurface->format->Amask);
    SDL_BlitSurface(biconsurface, &biconrects[id], image, NULL);
    return image;
}

/* Deallocate the button icons surface once the program has finished
 * initializing.
 */
void finalizeimages(void)
{
    SDL_FreeSurface(biconsurface);
    biconsurface = NULL;
}
